...
public class ActivitySearchResource {
	...
	public Response searchForActivities(ActivitySearch search){
		...
		List<Activity> activities = activityRepository.findConstraints(search);
	...
	@GET
	@Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
	public Response searchActivities(@QueryParam(value = "description") List<String> description, 
			@QueryParam(value ="durationFrom") int durationFrom,
			@QueryParam(value="durationTo") int durationTo) {
		
		System.out.println(description+","+durationFrom+","+ durationTo);
